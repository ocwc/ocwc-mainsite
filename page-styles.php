<?php get_header(); ?>


<div class="row">
		<div class="large-12 columns">
			<h2>OCWC Style Components</h2>
			<hr />
		</div>
	</div>
<div class="row">    
  <div class="large-12 columns"><br>
    <h3>Main Menu</h3>
    <ul id="menu-navigation" class="menu">
      <li><a href="#p">Home</a></li>
      <li><a href="#">About</a><br>
		<ul class="sub-menu">
			<li id="menu-item" class="menu">
				<a href="#">What is Open Education</a></li>
          	<li id="menu-item" class="menu"> 
            	<a href="#">Vision & Mission</a></li>
            <li id="menu-item" class="menu">
				<a href="#">Member Listing</a></li>
             <li id="menu-item" class="menu">
				<a href="#">Board of Director</a></li>                      
             <li id="menu-item" class="menu">
				<a href="#">Staff Members</a></li>   
		</ul></li>
      <li><a href="#">News Updates</a></li>
      <li><a href="#">Courses</a></li>
      <li><a href="#">Community</a></li>
	  <li><a href="#">Contact</a></li>

    </ul>
  </div><div class="large-6 columns"><br>
    <h3>Navigation Menu</h3>
    <ul id="toplevel">
      <li><a href="#p">Home</a></li>
      <li><a href="#">About</a>
		<ul class="sub-menu">
			<li id="menu-item" class="menu">
				<a href="#">What is Open Education</a></li>
          	<li id="menu-item" class="menu"> 
            	<a href="#">Vision & Mission</a></li>
            <li id="menu-item" class="menu">
				<a href="#">Member Listing</a></li>
             <li id="menu-item" class="menu">
				<a href="#">Board of Director</a></li>                      
             <li id="menu-item" class="menu">
				<a href="#">Staff Members</a></li>   
		</ul></li>
      <li><a href="#">News Updates</a></li>
      <li><a href="#">Courses</a></li>
      <li><a href="#">Community</a></li>
	  <li><a href="#">Contact</a></li>

    </ul>
  </div><div class="large-6 columns"><br>
    <h3>Navigation Menu 2</h3>
    <ul id="toplevel2">
      <li><a href="#p">Home</a></li>
      <li><a href="#">About</a>
		<ul class="sub-menu">
			<li id="menu-item" class="menu">
				<a href="#">What is Open Education</a></li>
          	<li id="menu-item" class="menu"> 
            	<a href="#">Vision & Mission</a></li>
            <li id="menu-item" class="menu">
				<a href="#">Member Listing</a></li>
             <li id="menu-item" class="menu">
				<a href="#">Board of Director</a></li>                      
             <li id="menu-item" class="menu">
				<a href="#">Staff Members</a></li>   
		</ul></li>
      <li><a href="#">News Updates</a></li>
      <li><a href="#">Courses</a></li>
      <li><a href="#">Community</a></li>
	  <li><a href="#">Contact</a></li>

    </ul>
  </div>

</div>
<div class="row">
	<div class="large-12 columns">
    <h1>H1 Heading </h1>
	<h2>H2 Heading</h2>
	<h3>H3 Heading</h3>
    <h4>H4 Heading</h4>
	<h5>H5 Heading</h5><br>
    </div>	
</div>
 <!--Breadcrumb!-->
<div class="row">
	<div class="large-12 columns">
        <h2>Breadcrumbs</h2>
              <ul class="breadcrumb">
			<li><a href="#">Courses</a></li>
			<li><a href="#">Language</a></li>
			<li><a href="#">Course Title</a></li>
	
		</ul>
    </div>
</div>
<div class="row">
<div class="large-12 columns">
	    <h2>Paragraph w/ HR</h2>
    <p> Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum</p><br>
        <hr></hr>
      </div>
</div>
<div class="row">
	  <div class="large-6 columns">
	    <h2>Paragraph</h2>
	    <p> Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum</p>
      </div>
<div class="large-6 columns">
	    <h2>Emphasis</h2>
		<p class="muted">Fusce dapibus, tellus ac cursus commodo, tortor mauris nibh.</p>
		<p class="text-warning">Etiam porta sem malesuada magna mollis euismod.</p>
		<p class="text-error">Donec ullamcorper nulla non metus auctor fringilla.</p>
		<p class="text-info">Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis.</p>
		<p class="text-success">Duis mollis, est non commodo luctus, nisi erat porttitor ligula.</p>
      </div>
<div class="large-6 columns">
	    <h2>Bullets</h2>
	    <ul>
<li>Lorem ipsum dolor sit amet</li>
<li>Consectetur adipiscing elit</li>
<li>Integer molestie lorem at massa</li>
<li>Facilisis in pretium nisl aliquet</li>
<li>
Nulla volutpat aliquam velit
<ul>
<li>Phasellus iaculis neque</li>
<li>Purus sodales ultricies</li>
<li>Vestibulum laoreet porttitor sem</li>
<li>Ac tristique libero volutpat at</li>
</ul>
</li>
<li>Faucibus porta lacus fringilla vel</li>
<li>Aenean sit amet erat nunc</li>
<li>Eget porttitor lorem</li>
</ul>
  </div><div class="large-6 columns">
	    <h2>Blockquote Styles</h2>
	    <blockquote>
  <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
  <small>Someone famous <cite title="Source Title">Source Title</cite></small>
</blockquote>
<blockquote class="pull-right">
  <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
  <small>Someone famous <cite title="Source Title">Source Title</cite></small>
</blockquote>
      </div><div class="large-6 columns">
	    <h2>Ordered Styles</h2>
	<ol>
<li>Lorem ipsum dolor sit amet</li>
<li>Consectetur adipiscing elit</li>
<li>Integer molestie lorem at massa</li>
<li>Facilisis in pretium nisl aliquet</li>
<li>Nulla volutpat aliquam velit</li>
<li>Faucibus porta lacus fringilla vel</li>
<li>Aenean sit amet erat nunc</li>
<li>Eget porttitor lorem</li>
</ol>
      </div><div class="large-6 columns">
	    <h2>Bullets</h2>
	    <p> Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum</p>
</div>
        <div class="large-6 columns">
	    <h2>Images</h2>
	    <p><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb"></p>
      </div>
        <div class="large-6 columns">
	    <h2>Image Left</h2>
  	    <p> <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="left" class="imgthumb">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum</p>
        <p>Please join the Community College Consortium for Open Educational<a href="http://www.ocwconsortium.org/blog/2013/10/22/oct-30-oer-and-open-textbook-adoption-and-sustainability/freethetextbook/" rel="attachment wp-att-1723"><img class="alignleft size-medium wp-image-1723" title="Free the Textbook" src="http://www.ocwconsortium.org/components/com_wordpress/wp/wp-content/uploads/2013/10/freethetextbook-300x168.jpg" alt="Free the Textbook" width="300" height="168"></a> Resources (CCCOER) on <strong>Wed, Oct 30, noon Pacific, 3:00 pm EST</strong> for a webinar on how three statewide projects have established open education portals to expand student access and foster faculty innovation.&nbsp; Leaders from California, Florida, and the province of British Columbia will share successful strategies and challenges to the continued growth and adoption of their OER and open textbook collections.</p>
      </div>
      <div class="large-6 columns">
        <h2>Image Right</h2>
        <p> <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/Layer29.png" alt="" width="111" height="82" align="right" class="imgthumb">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum</p>
        <p>Please join the Community College Consortium for Open Educational<a href="http://www.ocwconsortium.org/blog/2013/10/22/oct-30-oer-and-open-textbook-adoption-and-sustainability/freethetextbook/" rel="attachment wp-att-1723"><img class="alignright size-medium wp-image-1723" title="Free the Textbook" src="http://www.ocwconsortium.org/components/com_wordpress/wp/wp-content/uploads/2013/10/freethetextbook-300x168.jpg" alt="Free the Textbook" width="300" height="168"></a> Resources (CCCOER) on <strong>Wed, Oct 30, noon Pacific, 3:00 pm EST</strong> for a webinar on how three statewide projects have established open education portals to expand student access and foster faculty innovation.&nbsp; Leaders from California, Florida, and the province of British Columbia will share successful strategies and challenges to the continued growth and adoption of their OER and open textbook collections.</p>
      </div>
</div>
<!--Tables!-->
<div class="row">
	<div class="large-12 columns">
	    <h2>Tables</h2>
	    <table width="100%" class="table table-hover">
              <thead>
                <tr>
                  <th width="44%" class="tableblue">Course Title</th>
                  <th width="18%">Language</th>
                  <th width="19%">Source</th>
                  <th width="19%">Details</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td class="tableblue">Course Title 1</td>
                  <td>English</td>
                  <td>Link 1</td>
                  <td><i class="icon-search"></i> details</td>
                </tr>
                <tr>
                  <td class="tableblue">Course Title 2</td>
                  <td>English</td>
                  <td>Link 2</td>
                  <td><i class="icon-search"></i> details</td>

                </tr>
                <tr>
                  <td class="tableblue">Course Title 3</td>
                  <td>German</td>
                  <td>Link 3</td>
                  <td><i class="icon-search"></i> details</td>

                </tr>
                <tr>
                  <td class="tableblue">Course Title 4</td>
                  <td>German</td>
                  <td>Link 4</td>
                  <td><i class="icon-search"></i> details</td>

                </tr>
              </tbody>
      </table>
      <div class="pagination">
          <a href="#" class="previous-page">« Previous Page</a>
          <a href="#" class="next-page">Next Page »</a>
              
      </div>      

	</div>
</div>
<!--Courses!-->
</div>
<div class="row">
  <h2>Courses</h2>
	<div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div><div class="large-3 columns"><img src="<?php echo get_stylesheet_directory_uri(); ?>/images/course.jpg" width="245" height="145" class="coursethumb">
	  <h4>Course Title</h4>
    <p></div>
</div>
<div class="row">
	<div class="large-6 columns">
	    <h2>Forms</h2>
	<input type="text"/>
      </div>
<div class="large-6 columns">
	    <h2>Buttons</h2>
<button type="submit" class="button">Submit</button>
      </div>
 </div>

<?php get_footer(); ?>